...
- [x] Web Workerへ画像データを渡し、変換・キャンセル・資源解放を試作する
...
- [x] ファイル選択とドラッグ＆ドロップを実装する
- [ ] MIME、magic bytes、サイズ、画素数による入力検証を実装する
- [ ] 画像情報と入力プレビューを表示する
- [x] 出力形式、品質、幅、高さ、縦横比、背景色の設定を実装する
- [x] decode → orientation → resize → composite → encodeパイプラインを実装する
- [x] Workerとのメッセージ契約と構造化エラーを実装する
- [ ] 進捗、キャンセル、失敗、再試行を実装する
- [ ] 結果プレビュー、比較情報、個別ダウンロードを実装する
- [x] Blob URLなどの資源解放を実装する
...
- [x] 複数ファイルの追加、削除、全件クリアを実装する
- [x] 共通設定と変換キューを実装する
- [x] 端末負荷を考慮した同時実行数制御を実装する
- [x] 画像ごとの状態と全体進捗を表示する
- [x] 1件の失敗から他ジョブを分離する
- [x] ファイル名衝突を解決する
- [x] 成功結果をZIPへまとめ、一括ダウンロードできるようにする
